            ...
            <div className="container">
                  { 
                        todoList.map((todo: toDo) => {                            // переберет массив todoList(todos) и вернет компонент ListItem для каждого элемента массива
                              return ( <ListItem  key={todo.id}  todo={todo} /> )                    // вернет компонент. Передаем todo как аргумент(пропс) в фукнцию ListItem.  Указываем  key итерирумемому элементу, иначе ругаться будет
                        }) 
                  }
            </div>
      )
