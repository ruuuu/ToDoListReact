...
export const ViewListItem = () => {
      ...
      const todoList = useSelector((state: RootState) => state.todoList.todos);             // с помоью хука, получили массив todos из redux      

      
      useEffect(() => {                                            // хук, принимает  коллбэк, коллбэк будет запущен вовремя загрузки компонента. 
            //Нужен для инициализации(отбработает при инициализации  компонента и монтирования(добавления)), ренддера, дестроя компонента. Отрабоатет минимум 1 раз
